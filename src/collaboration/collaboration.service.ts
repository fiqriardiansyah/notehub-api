...
export class CollaborationService {
  ...
  async invite(user: User, invitation: InvitationData) {
    if (invitation.email === user.email) {
      throw new HttpException(
        'Invited account cannot be the same as the inviters',
        HttpStatus.BAD_REQUEST,
      );
    }

    const findCollab = (await this.prismaService.$queryRaw(
      Prisma.raw(`
    ...
    if (findCollab && findCollab?.length) {
      throw new HttpException(
        'This Account already in this project',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    const isPass1hour =
      new Date().getTime() - new Date(findInvitation?.updatedAt).getTime() >
      60 * 60 * 1000;

    if (findInvitation && findInvitation?.attempted >= 3) {
      throw new HttpException(
        `you have exceeded the invitation limit for ${invitation.email} on this project, make sure check the recipients email spam`,
    ...
    if (findInvitation && findInvitation?.updatedAt && !isPass1hour) {
      throw new HttpException(
        'Try again later in 1 hour',
        HttpStatus.PRECONDITION_FAILED,
      );
    }

    const token = generateToken();
    const html = await this.mailerTemplateService.compileTemplate(
      'collab-invitation',
      {
  ...
  async validateInvitationFromNotif(data: {
  ...
  async validateInvitation(
    ...
    if (status === 'accepted') {
      const invitedUser = await this.prismaService.user.findFirst({
        where: { email: findInvitation.invitedEmail },
      });
      if (!invitedUser) {
  ...
  async collabAccount(user: User, noteId: string) {
    const collabs = await this.prismaService.$queryRaw(
      Prisma.raw(`
