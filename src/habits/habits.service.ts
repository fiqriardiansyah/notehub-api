...
import { HabitsHistory, Note, Prisma, Timer } from '@prisma/client';
import { UserSession } from '@thallesp/nestjs-better-auth';
import { PrismaService } from 'src/common/prisma.service';
import { TodoDto } from 'src/models/todo.dto';

type RunningTimer = Pick<
  Timer,
  'id' | 'endTime' | 'startTime' | 'itemId' | 'isZenMode'
> &
  Pick<Note, 'title'> & {
    todos: TodoDto[];
    noteId: string;
...
export class HabitsService {
  ...
  async setZenMode(id: string, val: boolean) {
    if (val) {
      const alreadyZenMode = await this.prismaService.timer.findFirst({
        where: { isZenMode: true },
      });

      if (alreadyZenMode) {
        throw new HttpException(
          'There is timer already in zen mode',
  ...
  async getRunningTimer(user: User) {
    let rows = (await this.prismaService.$queryRaw(
      Prisma.raw(`
            select 
    ...
    rows = rows.map((row) => ({
      ...row,
      todos: row.todos.map((t) => JSON.parse(t as any)),
    }));
